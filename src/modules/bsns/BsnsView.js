...
export default function BsnsScreen(props) {    
    ...
    const _ImageUpload = (dlvrySq) => {
        props.setDlvrySq(dlvrySq);
    ...
    const _renderItem = ({item}) => {
        return (            
            <View style={styles.container}>
                <TouchableOpacity 
                    onPress={() => _ImageUpload(item.dlvrySq)}
                    style={styles.listContainer}>
                    <View style={styles.left}>
                        <View style={{ flexDirection: 'row' }}>
                            <Text style={styles.listTitle}>{item.bcncNm} </Text>
                            {item.rcvordNo?<Text>[ {item.rcvordNo} ]</Text>:null}
                        </View>
                        
                        <View style={styles.subTitle}>
                            <Text style={styles.subTitleText}>납기일 : {item.prdPymntDt}</Text>
                            {item.dlvryPlanDt !== null?<Text style={styles.subTitleText}>   |   출하계획일 : {item.dlvryPlanDt}</Text>:null}
                        </View>
                        
                    </View>
                    <View style={styles.right}>
                        {item.data2 !== ''?<Text style={styles.rightText}>{item.localNm}</Text>: null}
                    </View>
                </TouchableOpacity>            
            </View>
        )
        ...
        <View style={styles.container}>
            <View style={styles.header}>
                <View style={styles.title}>
                    <Text style={styles.titleText}>출하 현황</Text>
                </View>
            ...
            { props.filterVisible &&
                <View style={styles.filter}>
                    <Text style={styles.filterTitle}>물류구분</Text>
                
                    <View style={styles.filterButtons}>
                        {props.dstrbFilterList.map((item, index) => (
                            <View
                                style={{ flex: 1, marginRight: index===2? 0 : 5 }}
                                key={index}>
                                <TouchableOpacity
                                    style={item.code===props.dstrbFilter?styles.selectButton:styles.defaultButton}
                                    onPress={() => props.selectDstrbFilter(item.code)}>
                                    <Text style={item.code===props.dstrbFilter?styles.selectText:styles.defaultText}>{item.codeNm}</Text>
                                </TouchableOpacity>
                            </View>
                        ))}
                    </View>
            ...
            { props.error
                ...
                :(<FlatList
                    ...
                    data={props.bsnsList}
                    keyExtractor={(item) => '' + item.dlvrySq}
